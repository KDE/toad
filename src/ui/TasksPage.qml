...
Kirigami.ScrollablePage {
    ...
    title: {
        if (page.searching) {
            return i18np("1 result", "%1 results", list.count);
        } else {
            return i18nc("ratio of completed items to total", "%1/%2 completed", TasksModel.completedTasks, list.count) + " — " + TasksModel.name;
        }
    }
